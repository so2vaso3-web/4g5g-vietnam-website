      ...
      >
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4 sm:mb-6 pb-4 sm:pb-6 border-b border-white/10">
          <div>
            ...
            <p className="text-gray-400 text-xs sm:text-sm mt-1">
              So sánh {packages.length} gói cước cạnh nhau
            </p>
          </div>
          <button
            onClick={onClose}
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="border-b border-white/10">
                  <th className="text-left p-3 sm:p-4 font-semibold text-gray-300 sticky left-0 bg-[#1a1f3a] z-10 text-xs sm:text-sm">Tính Năng</th>
                  {packages.map(pkg => (
                    <th key={pkg.id} className="text-center p-3 sm:p-4 min-w-[200px] sm:min-w-[250px] relative">
                      ...
                      <div className="mb-2">
                        ...
                        <div className="text-2xl sm:text-3xl font-bold bg-gradient-to-r from-red-600 to-blue-600 bg-clip-text text-transparent mb-1">
                          {pkg.price.toLocaleString('vi-VN')}₫
                        </div>
                        <div className="text-xs sm:text-sm text-gray-400">/ {pkg.validity || 'tháng'}</div>
                      </div>
                    </th>
                  ))}
                </tr>
                ...
                <tr className="border-b border-white/5">
                  ...
                  {packages.map(pkg => (
                    <td key={pkg.id} className="p-3 sm:p-4 text-center">
                      <div className="text-lg sm:text-xl font-bold">{pkg.price.toLocaleString('vi-VN')}₫</div>
                      <div className="text-xs sm:text-sm text-gray-400">/ {pkg.validity || 'tháng'}</div>
                    </td>
                  ))}
                </tr>
        ...
        <div className="mt-6 sm:mt-8 pt-4 sm:pt-6 border-t border-white/10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div className="text-xs sm:text-sm text-gray-400">
            ...
            Chọn tối đa 4 gói cước để so sánh
          </div>
          <button
            onClick={onClose}
            className="w-full sm:w-auto px-4 sm:px-6 py-2.5 sm:py-3 bg-gradient-to-r from-red-600 to-blue-600 rounded-lg sm:rounded-xl font-semibold hover:from-red-700 hover:to-blue-700 transition-all duration-300 shadow-lg hover:shadow-blue-500/50 flex items-center justify-center gap-2 min-h-[44px]"
          >
