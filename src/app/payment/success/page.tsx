...
import { updateOrderOnServer } from '@/lib/useOrders';
  ...
  useEffect(() => {
    ...
    if (finalOrderId) {
      ...
      if (orderIndex !== -1 && orders[orderIndex].status === 'pending') {
        ...
        // QUAN TRỌNG: Cập nhật lên server để đồng bộ với tất cả thiết bị
        updateOrderOnServer(finalOrderId, { status: 'completed' }).catch(err => {
          console.error('Failed to update order on server:', err);
          // Fallback: vẫn lưu vào localStorage nếu server lỗi
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#0a0e27] via-[#1a1f3a] to-[#0a0e27] flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-white/5 backdrop-blur-md rounded-3xl p-8 border border-white/20 shadow-2xl text-center">
        ...
        <div className="flex flex-col gap-3">
          <Link
            href="/"
            className="px-6 py-3 bg-gradient-to-r from-red-600 to-blue-600 rounded-xl font-semibold hover:from-red-700 hover:to-blue-700 transition-all duration-300 shadow-lg hover:shadow-blue-500/50 flex items-center justify-center gap-2"
          >
