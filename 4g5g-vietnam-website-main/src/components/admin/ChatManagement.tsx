...
export default function ChatManagement() {
  ...
  useEffect(() => {
    loadMessages();
    // Polling để cập nhật - Tối ưu: 2s để giảm bandwidth nhưng vẫn đủ nhanh
  ...
  const sendReply = async (conversation: Conversation) => {
    ...
    const reply: Message = {
      ...
      name: 'Admin',
      email: 'admin@system', // Đặt giá trị mặc định thay vì empty string
    ...
    // Save to server - ĐẢM BẢO TIN NHẮN ADMIN ĐƯỢC LƯU NGAY (với retry)
    const maxRetries = 3;
    let lastError: string = '';
    const saveToServer = async (attempt: number = 1): Promise<boolean> => {
      try {
        console.log(`Attempting to save ${updatedWithRead.length} messages (attempt ${attempt}/${maxRetries})...`);
        ...
        if (!saveResponse.ok) {
          ...
          const errorMsg = `HTTP ${saveResponse.status}: ${saveResponse.statusText}. ${errorText}`;
          console.error('Failed to save reply to server:', errorMsg);
          lastError = errorMsg;
          throw new Error(errorMsg);
        }
        
        const result = await saveResponse.json();
        console.log('Reply saved to server successfully:', result);
        return true;
      } catch (error: any) {
        const errorMsg = error?.message || String(error) || 'Unknown error';
        console.error(`Error saving reply to server (attempt ${attempt}/${maxRetries}):`, errorMsg);
        lastError = errorMsg;
        if (attempt < maxRetries) {
          // Retry sau 500ms, 1000ms, 2000ms
          await new Promise(resolve => setTimeout(resolve, 500 * attempt));
          return saveToServer(attempt + 1);
        }
        return false;
      }
    };
    ...
    if (!saveSuccess) {
      setAlertModal({ 
        isOpen: true, 
        message: `Lỗi: Không thể lưu tin nhắn lên server sau ${maxRetries} lần thử!\n\nChi tiết: ${lastError}`, 
        type: 'error' 
      });
    } else {
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6 flex-wrap gap-4">
        <div>
          <h2 className="text-3xl font-bold mb-2 bg-gradient-to-r from-red-600 to-blue-600 bg-clip-text text-transparent">
                        ...
                        <div className="flex items-center justify-between mb-1">
                          ...
                          {msg.isAdmin && (
                            <div className="flex items-center gap-1 text-xs opacity-80">
                              {msg.read ? (
                                <>
                                  <i className="fas fa-check-double text-green-300"></i>
                                  <span className="text-green-300">Đã đọc</span>
                                </>
                              ) : (
                                <>
                                  <i className="fas fa-check text-gray-300"></i>
                                  <span className="text-gray-300">Đã gửi</span>
                                </>
                              )}
                            </div>
                          )}
                ...
                <button
                  ...
                  className={`w-full px-4 py-3 rounded-lg font-semibold transition-all duration-300 flex items-center justify-center gap-2 ${
                    replyText.trim() && selectedConversation
                      ? 'bg-gradient-to-r from-red-600 to-blue-600 text-white hover:from-red-700 hover:to-blue-700 shadow-lg hover:shadow-blue-500/50 cursor-pointer'
